...
private[insightedge] case class InsightEdgeDocumentRelation(
  ...
  private def getStructType(collection : String): StructType = {
    ...
    var properties = typeDescriptor.getPropertiesNames
    println("COLLECTION= " + collection +
      "Prop=" + properties)
    var structType = new StructType()

//    if (typeDescriptor.isAutoGenerateId) {
//      properties = properties.filterNot(autogeneratedIdProperty => autogeneratedIdProperty == typeDescriptor.getIdPropertyName )
//    }

    for (a <- properties) {
      val property = typeDescriptor.getFixedProperty(a)
